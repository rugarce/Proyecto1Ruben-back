 ...
 * 
 * Esta interfaz define los métodos para realizar operaciones CRUD sobre la
 * entidad Producto.
 */
public interface ProductoProvider {
	 ...
	 * 
	 * @param producto Un objeto ProductoDto que contiene la informacion del
	 *                 producto a crear.
	 ...
	 * @param producto Un objeto ProductoDto que contiene la información actualizada
	 *                 del producto.
	...
	/**
	 * Obtiene todos los productos.O
	 * 
	 * @return Una lista de entidades producto que representa todos los productos.
	 */
	List<ProductoAllDto> allProducts();

	PaginadoDto<ProductoAllDto> obtenerProductosPaginados(int numPagina, int tamanoPagina, String nombre, List<String> marcas,
			List<String> tiendas,List<String> proveedores, Float precio, Integer cantidad);

	List<ProductoAllDto> buscarProductos(String nombre, String marca, String tienda, Float precio, Integer cantidad);
}
